...
<header>
    <div class="container">
        ...
        {
            $dataInizio = htmlspecialchars($_GET['dataInizio']); // Protezione XSS
            $dataFine = htmlspecialchars($_GET['dataFine']); // Protezione XSS
            
            if (!$dataInizio||!$dataFine && $dataInizio<$dataFine ) {
                echo "<h1 class='text-danger'>Errore: formato data non valido.</h1>";
                exit();
            }

        
            $params = array($dataInizio, $dataFine );
            
            // Query per contare il numero di record
            $count_query = "SELECT COUNT(*) AS totale FROM Immagini WHERE Giorno BETWEEN ? AND ?";
            ...
            if ($row_count = sqlsrv_fetch_array($stmt_count, SQLSRV_FETCH_ASSOC)) 
            {
                if ($row_count['totale'] > 0) 
                {
                    ...
                    include 'query.php';
                    $query = "SELECT Giorno, NDVI, Foto FROM Immagini WHERE Giorno BETWEEN ? AND ?";
                ...
                else 
                {
                    ...
                    <div class='d-flex flex-column justify-content-center align-items-center min-vh-100'>
                        <h1 class='text-danger fs-1 mb-4 text-center'>Nessuna immagine disponibile tra ". htmlspecialchars($dataInizio) ." e il ".htmlspecialchars($dataFine) ."</h1>
                        <a href='viewbetweendate.html' class='btn btn-primary btn-lg'>Torna alla ricerca</a>
                    </div>";
                }
            }
